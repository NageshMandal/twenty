...
import MenuButtonBuilder from '../../../../components/base/MenuButtonBuilder';
...
const AddActionNode = ({ data, id }: { data: NodeData; id: string }) => {
  ...
  const [isNewNode, setIsNewNode] = useState(
    data.isNewNode === undefined ? true : data.isNewNode,
  );
  ...
  useEffect(() => {
    ...
    fieldsToUpdate.forEach((fieldName: FieldsToUpdate) => {
      if (data[fieldName as keyof NodeData]) {
        ...
        data.formData[fieldName as keyof NodeData] =
          data[fieldName as keyof NodeData];
      }
    });
    ...
    if (builderMessage) {
      ...
      if (selectedWorkflow?.title == 'AI SDR') {
        if (builderAiSdr.id === id) {
          ...
          data.formData = {
            aiSdrAdditionalInfoField: builderAiSdr.aiSdrAdditionalInfoField,
          };
          data.formData = {
            aiSdrObjectionsField: builderAiSdr.aiSdrObjectionsField,
          };
          data.formData = {
            aiSdrCompetitionsField: builderAiSdr.aiSdrCompetitionsField,
          };
        ...
        if (builderAiSdrSetup.id === id) {
          data.formData = {
            aiSdrSetupSystemPrompt: builderAiSdrSetup.aiSdrSetupSystemPrompt,
          };
          data.formData = {
            aiSdrSetupPrompt: builderAiSdrSetup.aiSdrSetupPrompt,
          };
          data.aiSdrSetupSystemPrompt =
            builderAiSdrSetup.aiSdrSetupSystemPrompt;
      ...
      {!data?.isDelay ? (
          ...
          button={
            <div
              className={`cursor-pointer h-full text-white items-center px-10 flex justify-center ${
                data.isInitialNode || data.isDropDownDisabled
                  ? 'bg-primary'
                  : 'bg-gray-400'
              } ${
                data.isInitialNode
              ...
              <p className="px-6">
                {selectedOption === 'Add Action' ? (
                  ...
                  <span className="flex items-center">
                    ...
                    <Icon
                      name={
                        (selectedWorkflow?.icon as IconType) ?? 'LogoBuilder'
                      }
        ...
        >
          <p className="px-6">
            ...
            {" - "} */}
            {data.delayValue
              ? `Wait for ${data.delayValue} ${data.delayUnit?.value}`
              : 'Delay'}
          </p>
        </div>
      )}
