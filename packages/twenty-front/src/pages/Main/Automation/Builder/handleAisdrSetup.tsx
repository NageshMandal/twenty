...
export function handleAisdrSetup(aisdrData, newWorkflowName) {
  ...
  if (
    ...
    const departmentHeadCount =
      aisdrData.defineIcpAndPersona.departmentHeadCount || '';
    ...
    const includeRecentlyHired =
      aisdrData.defineIcpAndPersona.includeRecentlyHired || false;
    const contactLocation = aisdrData.defineIcpAndPersona.contactLocation || [];
    const companiesToProspect =
      aisdrData.defineIcpAndPersona.companiesToProspect || 0;
    const prospectsPerCompanyPerUser =
      aisdrData.defineIcpAndPersona.prospectsPerCompanyPerUser || 0;
    const useDirectPathToProspect =
      aisdrData.defineIcpAndPersona.useDirectPathToProspect || false;
    const visitorIdentified =
      aisdrData.defineIcpAndPersona.visitorIdentified || false;
    ...
    const companySizeByEmployee =
      aisdrData.defineIcpAndPersona.companySizeByEmployee || 0;
    const firstSystemPrompt =
      aisdrData.personalisationOptions.typesAndSignals[0].data.systemPrompt
        .value || '';
    const firstPrompt =
      aisdrData.personalisationOptions.typesAndSignals[0].data
    ...
    if (
      ...
      workflow.flowchart.push({
        ...
        prevNode: aisdrData.personalisationOptions.outboundOnLinkedIn
          ? node14UId
          : node8UId,
      });
      workflow.flowchart.push({
