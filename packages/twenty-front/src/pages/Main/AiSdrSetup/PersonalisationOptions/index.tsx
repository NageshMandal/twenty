...
import SidebarEdit, {
  FormData as SidebarEditFormData,
  defaultTypesAndSignalsData,
} from './SidebarEdit';
...
const PersonalisationOptions: React.FC<Props> = ({ setCurrentTab, onSave }) => {
  const [selectedTypeAndSignal, setSelectedTypeAndSignal] = useState<
    string | null
  >(null);
  ...
  const outboundOnLinkedIn = useWatch({
    control,
    name: 'outboundOnLinkedIn',
    defaultValue: true,
  });
  const outboundOnEmail = useWatch({
    control,
    name: 'outboundOnEmail',
    defaultValue: true,
  });
  ...
  const handleInputChange = (e: ISelectOption) => {
    setFormData({ ...formData, [e.label]: e.value });
  };

  const handleOrderOfTypesAndSignals = (value: string) => {
    const typeAndSignal = formData.typesAndSignals.find(
      (v) => v.title === value,
    );
  ...
  const handleUpdateTypesAndSignalsOrders = (
    typesAndSignalsOrderedTitles: string[],
  ) => {
    setFormData({
      ...formData,
  ...
  const handleEdit = (data: SidebarEditFormData) => {
    ...
    setFormData({
      ...
      typesAndSignals: formData.typesAndSignals.map((typeAndSignal) =>
        typeAndSignal.title === selectedTypeAndSignal
          ? { ...typeAndSignal, data }
          : typeAndSignal,
      ),
    });
  ...
  const handleRemoveTypeAndSignal = (typeAndSignal: string) => {
    setFormData({
      ...formData,
      typesAndSignals: formData.typesAndSignals.filter(
        (v) => v.title !== typeAndSignal,
      ),
    });
  };
  ...
  return (
    <div className="w-full">
          ...
          <LanguageOfCampaign
            ...
            value={
              formData.languageOfCampaign
                ? {
                    label: formData.languageOfCampaign,
                    value: formData.languageOfCampaign,
                  }
                : undefined
            }
      ...
      <SidebarEdit
        data={formData.typesAndSignals.find(
          (v) => v.title === selectedTypeAndSignal,
        )}
