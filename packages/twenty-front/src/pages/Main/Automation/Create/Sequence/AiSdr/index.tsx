...
import { aiSdrVariables } from '../../../../../../pages/Main/Automation/Builder/TemplateFormOptions';
...
const AiSdr: React.FC<Props> = ({ builderAiSdr }) => {
  ...
  const textAreaValue2 = useWatch({
    control,
    name: 'aiSdrAdditionalInfoField',
  });
  const textAreaValueObjections = useWatch({
    control,
    name: 'aiSdrObjectionsField',
  });
  const textAreaValueCompetitions = useWatch({
    control,
    name: 'aiSdrCompetitionsField',
  });
  ...
  const [characterCount1, setCharacterCount] = useState(
    textAreaValue1?.length || 0,
  );
  const [characterCount2, setCharacterCount2] = useState(
    textAreaValue2?.length || 0,
  );
  ...
  useEffect(() => {
    ...
    setValue(
      ...
      builderAiSdr?.aiSdrAdditionalInfoField,
    );
  ...
  useEffect(() => {
    ...
    textAreaValue1,
    textAreaValue2,
    textAreaValueObjections,
    textAreaValueCompetitions,
  ]);
            ...
            <div className="grid grid-cols-2 gap-40">
              <div className="flex flex-col gap-12">
                <ReactSelect
                  ...
                  value={aiSdrVariables.find(
                    (item) => item.value === aiSdrPitchCTAId,
                  )}
                ></ReactSelect>
                <Textarea
                  label="Pitch"
                ...
                <div>
                  <div className="flex flex-col justify-start gap-6 p-8 py-12 border shadow-md bg-contentColor dark:bg-contentColor-dark border-borderColor dark:border-borderColor-dark rounded-md">
                    ...
                    <p className="text-14 title-1">
                      When this Option is ON the system will automatically Reply
                      on your behalf. when OFF you can review them prior to the
                      Manual Reply
                    </p>
                  </div>
                </div>
