      ...
      {currentMethod ? (
        ...
        <>
          ...
          <div className="flex flex-wrap items-center justify-center gap-30">
            {blocks.map((item) => (
              <Tooltip key={item.value} label={item.comingSoon ? "Coming Soon" : item.title}>
                <div
                  className={`relative flex items-center justify-center transition-all duration-200 border-2 cursor-pointer rounded-2xl w-300 h-210 border-primary-1 hover:brightness-90 ${
                    item.comingSoon ? "opacity-50 cursor-not-allowed" : "opacity-100"
                  }`}
                  onClick={() => {
                    if (!item.comingSoon) {
                ...
                >
                  ...
                  <div className="flex flex-col items-center justify-center gap-14">
                    ...
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <p className="text-12 text-neutral-900 dark:text-neutral-300">
                        {item.description}
                      </p>
                    </div>
                  </div>
