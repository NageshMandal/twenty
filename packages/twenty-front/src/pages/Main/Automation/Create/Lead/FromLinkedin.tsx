...
const FromLinkedin: React.FC<Props> = ({
  onNext,
  onBack,
  ...
  const [selectedStopCondition, setSelectedStopCondition] =
  ...
  const handleUrl = async (e: any) => {
    ...
    // eslint-disable-next-line @nx/workspace-explicit-boolean-predicates-in-if
      ...
      (updatedWorkflow.flowchart[0].content as any).extractProfileNumber =
        profileNumber;
      setIsFormValid(true);
      ...
      (updatedWorkflow.flowchart[0].content as any).numberOfMembers =
        profileNumber;
      setIsFormValid(true);
    } else {
