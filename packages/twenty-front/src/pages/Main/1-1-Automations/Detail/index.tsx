...
import { getAskAiPrompts } from '../../../../store/SocialSelling/actions';
...
const OneAutomationPage: React.FC = () => {
  ...
  const { prospects, isProspectsPending } = useAppSelector(
    personalizationSelector,
  );
          ...
          {!isLoading && (
            <>
              <p className="text-14 file:font-normal text-neutral-800 dark:text-neutral-300">
                {postLength} posts of{' '}
                {prospects?.length ? prospects?.length : 0} prospects found in
              </p>
